...
const definitions = [
  { category: "Privacy Model", entries: [
    ...
    { term: "Secretless (no per-note keys)", def: "Users handle a single signing key (or none, via AA). No per-note secrets to manage or lose. STRK20s derives all keys from the Starknet account key via account abstraction. Aztec requires 3 separate key pairs. Aleo requires private key + view key. Railgun requires spending + viewing keys. Monero requires spend + view keys." },
    { term: "Efficient note discovery", def: "Recipients discover funds in time proportional to their own activity. STRK20s uses on-chain indexed storage with shared secrets. Aztec uses a tagging mechanism but requires pre-registering senders. Zcash requires scanning all blocks. Railgun and Aleo use standard record/UTXO scanning." },
  ...
  { category: "Performance & UX", entries: [
    { term: "Fast shield/unshield", def: "How quickly a user can move between public and private states. STRK20s: sub-2s block + sub-5s proof. Zcash: ~75s block bottleneck. Monero: always private, no opt-out. Aztec: 36\u201372s blocks in Alpha. Railgun: 1-hour mandatory standby (PPOI). Solana: feature disabled. Zama: FHE much slower." },
    ...
    { term: "Scalable private transactions", def: "High throughput and low cost for private operations. Starknet: ~1,000 TPS today, targeting 10k+, $0.002 avg fee. Canton: horizontal scaling but access-control model. Aztec: ~1 TPS in Alpha. Zcash/Monero: PoW L1s. Railgun: inherits L1 gas costs." },
    { term: "Programmable privacy", def: "Ability to prove complex logic privately, verified cheaply on-chain. STRK20s: Cairo + Stwo enables proving entire block logic. Aztec: Noir + PXE. Aleo: Leo but no cross-program composability. Zcash: simple transfers only. Tornado Cash: fixed-denomination deposits were the ceiling." },
  ]},
